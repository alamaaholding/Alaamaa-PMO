...
console.log('\n▸ `.hidden` ليست بديلًا عن style="display:none"');
{
  ...
  for (const d of ['src', 'src/app']) for (const f of fs.readdirSync(d)) {
    ...
    // والسمتان بأيّ ترتيب: `id` ثم `class` **أو** العكس. وترتيبٌ واحدٌ كان
    // يترك النصفَ الآخر بلا حارس — وقد شُحن عطبٌ من ذاك النصف: عنصرٌ يُولَد
    // `<div class="is-hidden" id="chnCustomFields">` ويُظهَر بـ`style.display=''`،
    // فيبقى مخفيًّا لأن الصنف لا يُغلَب بإزالة أسلوبٍ سطريّ. (لوحةُ «نص
    // مخصَّص» في إنشاء العقد لم تكن تظهر إطلاقًا.)
    const HID = String.raw`class="[^"]*(?<![\w-])(?:is-)?hidden(?![\w-])[^"]*"`;
