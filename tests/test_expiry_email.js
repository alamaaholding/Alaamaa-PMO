...
const wait=setInterval(()=>{
  ...
  const extra=[
    ...
    ['زر الإرسال يظهر بعد الاعتماد الداخلي وقبل توقيع الشريك فقط',
      hub.includes("(c.internal_approved&&c.status!=='void'&&!cl)?")],
    ['الزر يتحول لتذكير بعد أول إرسال',hub.includes("c.send_count>0?'🔔 إرسال تذكير'")],
    ['آخر موعد إرسال يظهر للمستخدم',hub.includes('آخر إرسال:')],
