...
const LONGEST_FN_CAP = 143;   // W3: 815 ← … ← 165 ← 145 (صفحة التوقيع العامّة). الهدف ≤ 120.
// وعند ٢٢٤ لم تعد لوحة العقد أطول دالة؛ ثم ٢٠٨ بخروج أدوات المكتب من المحفظة.
{
  const files = [
    ...fs.readdirSync('src').filter(f => f.endsWith('.js') && f !== 'qrgen.js').map(f => `src/${f}`),
    ...fs.readdirSync('src/app').filter(f => f.endsWith('.js')).map(f => `src/app/${f}`)
...
const INLINE_STYLE_CAP = 169;   // W0=372 · W5=322 · W4=203 · W6=198 (.hidden) ← 169 (صفحة التوقيع وعقود المشروع)
