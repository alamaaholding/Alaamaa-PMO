...
// هو ما كان غيابه يسمح بالنموّ أصلًا.
{
  ...
  const CORE_KB_CAP = 397;   // W2: 621 ← … ← 404؛ W3: 405 ← 401 (byId) ← 400 ($$)؛ W6: 399 ← 397 (أنماط→أصناف). الهدف < 250.
...
// ═══ ثابتٌ يُستعمَل عند التحميل قبل أن يُعرَّف ═══
//
// كتبتُ `HUB_FILTER_DEFAULT` **بعد** السطر الذي يستعمله في تهيئة الوحدة. ولم
// يرمِ شيء: esbuild يُخرج `var` لا `const` (قرارٌ في build.py: المُعرِّفات
// تبقى كما هي)، والرفعُ يُعطي `undefined` بدل أن يرمي. فصار
// `Object.assign({},undefined)` = `{}` — وشريطُ الفلاتر لا يُعلِّم ترتيبًا ولا
// حالة، بينما القائمة تحته مرتَّبةٌ فعلًا.
//
// **ومرّت الاختبارات كلُّها.** فالحارسُ يقيس الحزمة نفسها: موضعُ التعريف قبل
// موضع أوّل استعمال. وهذا يمسك الصنف كلَّه لا هذه الحالة وحدها.
{
  const PAIRS = [['HUB_FILTER_DEFAULT={status', 'HUB_FILTER_DEFAULT)']];
  for (const [def, use] of PAIRS) {
    const d = bundle.indexOf(def), u = bundle.indexOf(use);
    t(`${def.split('=')[0]} يُعرَّف قبل أوّل استعمال`,
      d > -1 && u > -1 && d < u, `تعريف ${d} · استعمال ${u}`);
  }
}
